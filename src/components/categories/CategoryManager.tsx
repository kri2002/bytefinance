...
export default function CategoryManager({ initialData }: CategoryManagerProps) {
  ...
  const handleSave = async (formData: CategoryFormValues) => {
    const isEdit = !!editingCategory;
    const tempId = editingCategory?.id || crypto.randomUUID();
    const newCat: Category = { id: tempId, ...formData };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <CategorySidePanel
        ...
        onSave={handleSave}
        initialData={editingCategory as unknown as CategoryFormValues | null}
      />

      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Layers className="text-purple-500" /> Categorías
          </h1>
          <p className="text-slate-400">
            Organiza tus movimientos con etiquetas.
          </p>
        </div>
        <button
          onClick={handleOpenNew}
          className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2.5 rounded-lg flex items-center gap-2 font-medium shadow-lg shadow-blue-900/20 transition-all hover:scale-105"
        >
          <Plus size={20} /> Nueva
        </button>
      </div>
        ...
        {categories.map((cat) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-full ${cat.color} flex items-center justify-center text-white font-bold text-lg shadow-inner`}
              >
