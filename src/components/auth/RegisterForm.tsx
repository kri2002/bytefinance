...
import { Mail, Lock, User, ArrowRight, Loader2, KeyRound, AlertCircle, CheckCircle2 } from 'lucide-react';
// 1. IMPORTAR FUNCIONES DE AWS
...
export default function RegisterForm() {
  ...
  const [code, setCode] = useState(''); // Para el código de verificación
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
        const { nextStep } = await signUp({
            ...
            options: {
                userAttributes: {
                    name: formData.name // Enviamos el nombre a Cognito
                }
            }
        });

        // Si AWS pide confirmar el correo (siempre pasa por defecto)
        ...
        if (err.name === 'UsernameExistsException') {
            ...
            setError('La contraseña es muy débil (usa mayúsculas, números y símbolos).');
        } else {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (isSignUpComplete) {
            // Éxito total. Redirigimos al Login para que entre.
            // (Opcional: Podrías usar autoSignIn aquí, pero ir al login es más seguro para empezar)
            router.push('/login'); 
        }
    } catch (err: any) {
  ...
  return (
    <div className="w-full flex items-center justify-center p-4">
      ...
      {/* FONDO DECORATIVO (Estilo Esmeralda) */}
      ...
      <div className="bg-slate-900/80 border border-slate-800 w-full max-w-md p-8 rounded-2xl shadow-2xl relative z-10 backdrop-blur-md animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* VISTA 2: FORMULARIO DE CÓDIGO */}
        {step === 'confirm' && (
            <form onSubmit={handleVerify} className="space-y-6 animate-in fade-in zoom-in-95">
                <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-300">Código de Verificación</label>
                    <div className="relative group">
                        <KeyRound className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-emerald-500 transition-colors" size={18} />
                        <input 
                            type="text" 
                            required 
                            placeholder="123456" 
                    ...
                    <p className="text-xs text-slate-500 text-center">Revisa tu bandeja de entrada o spam.</p>
                </div>

                <button type="submit" disabled={isLoading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-emerald-900/30 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
                    {isLoading ? <Loader2 className="animate-spin" size={20} /> : <>Verificar Cuenta <CheckCircle2 size={18} /></>}
                </button>
                
                <button type="button" onClick={() => setStep('register')} className="w-full text-sm text-slate-500 hover:text-white transition-colors">
