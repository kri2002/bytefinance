...
import { Plus, Wallet, CreditCard, Landmark, ArrowUpRight, ArrowDownRight, Trash2, Edit2 } from 'lucide-react';
...
// --- SUB-COMPONENTE TARJETA ORDENABLE ---
function SortableAccountItem({ account, onEdit, onDelete, formatCurrency }: { account: Account, onEdit: (acc: Account) => void, onDelete: (id: string, e: React.MouseEvent) => void, formatCurrency: (val: number) => string }) {
    
    // Hook de dnd-kit para hacer el elemento arrastrable
    ...
    return (
        <div
            ref={setNodeRef} style={style} {...attributes} {...listeners} // Conectamos el hook al div
            onClick={() => onEdit(account)}
            className={`relative overflow-hidden rounded-2xl p-6 h-52 flex flex-col justify-between transition-all hover:shadow-2xl shadow-lg group select-none cursor-grab active:cursor-grabbing border border-transparent hover:border-white/10 ${isDragging ? 'shadow-2xl scale-105' : ''}`}
        >
            {/* Fondo Gradiente */}
            <div className={`absolute inset-0 bg-linear-to-br ${account.color} opacity-90 group-hover:opacity-100 transition-opacity`} />
            ...
            <div className="relative z-10 flex justify-between items-start">
                ...
                <div className="flex gap-2">
                    ...
                    {/* Botón Borrar (con onPointerDown para evitar conflictos de drag) */}
                    <button 
                        onPointerDown={(e) => e.stopPropagation()} // Importante para que no inicie drag al clickar borrar
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-end">
                    ...
                    <div className="text-white/50 group-hover:text-white/80 transition-colors">
                            ...
                            account.type === 'debit' ? <Landmark size={24}/> : <CreditCard size={24}/>}
                    </div>
                </div>
            </div>
        </div>
...
export default function AccountsList({ initialData }: AccountsListProps) {
  ...
  // Sensores para detectar el arrastre (Mouse, Touch y Teclado)
  const sensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 8 } }), // distance: 8 evita que un simple click se detecte como drag
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      // NOTA: Aquí podrías llamar a una Server Action para guardar el nuevo orden en DB si quisieras persistencia del orden.
    }
  };

  // --- HANDLERS EXISTENTES ---
  const handleOpenNew = () => { setEditingAccount(null); setIsModalOpen(true); };
  const handleOpenEdit = (account: Account) => { setEditingAccount(account); setIsModalOpen(true); };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleSaveAccount = async (formData: any) => {
    const isEdit = !!editingAccount;
    const tempId = editingAccount?.id || crypto.randomUUID();
    const newAccount: Account = { id: tempId, ...formData };
    ...
    const res = await saveAccount({ ...formData, id: isEdit ? tempId : undefined });
    if(res.success) {
        Swal.fire({
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    const confirm = await Swal.fire({
        title: '¿Eliminar cuenta?', text: "Se borrará permanentemente.",
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <AccountFormModal 
        ...
        onSave={handleSaveAccount} initialData={editingAccount as any}
      />
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-6">
        ...
        <div className="flex gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl min-w-40">
                ...
                <p className={`text-xl font-bold mt-1 ${netWorth >= 0 ? 'text-white' : 'text-rose-400'}`}>{formatCurrency(netWorth)}</p>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl min-w-40">
                <p className="text-xs text-slate-500 font-medium uppercase flex items-center gap-1"><ArrowUpRight size={14} className="text-emerald-500"/> Activos</p>
                <p className="text-xl font-bold mt-1 text-emerald-400">{formatCurrency(totalAssets)}</p>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl min-w-40">
                <p className="text-xs text-slate-500 font-medium uppercase flex items-center gap-1"><ArrowDownRight size={14} className="text-rose-500"/> Deuda</p>
      ...
      <div className="flex justify-between items-center border-b border-slate-800 pb-4">
        ...
        <button onClick={handleOpenNew} className="flex items-center gap-2 text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">
            <Plus size={18} /> Agregar Cuenta
        </button>
      </div>

      {/* Grid de Tarjetas (AREA DE ARRASTRE) */}
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={accounts} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                
                {/* Mapeamos los items ordenables */}
