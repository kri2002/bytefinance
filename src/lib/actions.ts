...
/* CUENTAS (/accounts) */
export async function saveAccount(data: { id?: string; name: string; type: string; balance: number; bankName: string; last4?: string; color: string }) {
  try {
    const id = data.id || crypto.randomUUID();
...
export async function getAccounts() {
  try {
    ...
    if (!result.Items || result.Items.length === 0) {
        ...
        const defaults = [
            { name: "Didi Card", type: "debit", balance: 0, bankName: "Didi", color: "from-orange-600 to-orange-900" },
            { name: "Uber Card", type: "debit", balance: 0, bankName: "Uber", color: "from-gray-800 to-black" },
            { name: "Efectivo", type: "cash", balance: 0, bankName: "Físico", color: "from-emerald-600 to-emerald-900" }
        ];
...
export async function updateAccountBalance(accountName: string, amount: number) {
  try {
...
export async function saveCategory(data: { id?: string; name: string; color: string; type: 'expense' | 'income' }) {
  try {
    const id = data.id || crypto.randomUUID();
